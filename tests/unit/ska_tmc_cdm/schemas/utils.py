...
def test_schema_serialisation_and_validation(
    ...
    """
    test_marshal(schema_cls, instance, valid_json, validate)
    test_unmarshal(schema_cls, valid_json, instance, validate)
    test_serialising_valid_object_does_not_raise_exception_when_strict(
        schema_cls, instance, validate
    )
    ...
    if modifier_fn is not None:
        test_serialising_invalid_object_raises_exception_when_strict(
            schema_cls, instance, modifier_fn, validate
        )
    ...
    if invalid_json is not None:
        test_deserialising_invalid_json_raises_exception_when_strict(
            schema_cls, invalid_json, validate
        )


def test_marshal(schema_cls, instance, valid_json, validate=True):
    """
    ...
    schema = get_schema(schema_cls, validate, strictness=1)
    marshalled = schema.dumps(instance)
    assert_json_is_equal(marshalled, valid_json)


def test_unmarshal(schema_cls, valid_json, instance, validate=True):
    """
...
def test_deserialising_invalid_json_raises_exception_when_strict(
    schema_cls, invalid_json, validate=True
):
    """
...
def test_serialising_invalid_object_raises_exception_when_strict(
    schema_cls, instance, modifier_fn, validate=True
):
    """
...
def get_schema(schema_cls, validate=True, strictness: int = 0):
    schema = schema_cls()
    schema.context[ValidatingSchema.VALIDATE] = validate
