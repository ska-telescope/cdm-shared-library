...
def test_schema_serialisation_and_validation(
    ...
    """
    test_marshal(schema_cls, instance, valid_json, is_validate, is_semantic_validate)
    test_unmarshal(schema_cls, valid_json, instance, is_validate, is_semantic_validate)
    test_serialising_valid_object_does_not_raise_exception_when_strict(
        schema_cls, instance, is_validate, is_semantic_validate
    )
    ...
    if modifier_fn is not None:
        ...
        test_serialising_invalid_object_raises_exception_when_strict(
            schema_cls, instance, modifier_fn, is_validate, is_semantic_validate
        )
    ...
    if invalid_json is not None:
        test_deserialising_invalid_json_raises_exception_when_strict(
            schema_cls, invalid_json, is_validate, is_semantic_validate
        )


def test_marshal(
    schema_cls, instance, valid_json, is_validate=True, is_semantic_validate=True
):
    """
    ...
    schema = get_schema(schema_cls, is_validate, is_semantic_validate, strictness=1)
    marshalled = schema.dumps(instance)
    assert_json_is_equal(marshalled, valid_json)


def test_unmarshal(
    schema_cls, valid_json, instance, is_validate=True, is_semantic_validate=True
):
    """
...
def test_deserialising_invalid_json_raises_exception_when_strict(
    ...
    """
    schema = get_schema(schema_cls, is_validate, is_semantic_validate, strictness=2)
...
def get_schema(
    schema_cls, is_validate=True, is_semantic_validate=True, strictness: int = 0
):
    schema = schema_cls()
