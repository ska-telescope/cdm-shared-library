...
from ska.cdm.messages.central_node.sdp import ProcessingBlockConfiguration, PbDependency
from ska.cdm.messages.central_node.sdp import SDPWorkflow, SDPConfiguration, ScanType, Channel
...
def test_assign_resources_request_eq():
    ...
    assert request != AssignResourcesRequest(1, mccs_allocate=MCCSAllocate(2, [1,2,3,4], [1,2,3,4,5,6,7,8,9]))
    assert request != AssignResourcesRequest(1, mccs_allocate=MCCSAllocate(1, [3,4,5], [1,2,3,4,5,6,7,8,9]))
    assert request != AssignResourcesRequest(1, mccs_allocate=MCCSAllocate(1, [1,2,3,4], [1,2,3,4,5,6]))
