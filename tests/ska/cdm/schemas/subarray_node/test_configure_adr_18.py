...
def test_marshall_configure_request_no_cam():
    """
    Verify that ConfigureRequest is marshalled to JSON correctly
    when ChannelAveragingMap is not provided in FSP configuration.
    """
    scan_duration = timedelta(seconds=10)
    target = Target(
        ra="13:29:52.698",
...
def test_marshall_configure_request_olm():
    """
    Verify that ConfigureRequest is marshalled to JSON correctly
    when output link map is defined for FSP configuration.
    """
    scan_duration = timedelta(seconds=10)
    target = Target(
        ra="13:29:52.698",
...
def test_marshall_configure_request_fco():
    """
    Verify that ConfigureRequest is marshalled to JSON correctly
    when FSP Channel Offset is provided in FSP configuration.
    """
    scan_duration = timedelta(seconds=10)
    target = Target(
        ra="13:29:52.698",
    ...
    assert json_is_equal(request_json, VALID_CONFIGURE_REQUEST_FCO)


def test_unmarshall_configure_request_from_json_fco():
    """
    Verify that a ConfigureRequest can be unmarshalled from JSON
    when FSP Channel Offset is provided in FSP configuration.
    """
    ...
    unmarshalled = ConfigureRequestSchema().loads(VALID_CONFIGURE_REQUEST_FCO)
...
def test_marshall_configure_request_zoom():
    """
    Verify that ConfigureRequest is marshalled to JSON correctly
    when Zoom Window Tuning is provided in FSP configuration.
    """
    scan_duration = timedelta(seconds=10)
    target = Target(
        ra="13:29:52.698",
